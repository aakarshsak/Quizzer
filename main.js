// const bank = require('./bank.json');
// const fs = require('fs');

const bank = {
    "response_code": 0,
    "results": [
        {
            "category": "Science & Nature",
            "type": "multiple",
            "difficulty": "easy",
            "question": "Which of the following bones is not in the leg?",
            "correct_answer": "Radius",
            "incorrect_answers": [
                "Radius",
                "Patella",
                "Tibia",
                "Fibula "
            ],
            "id": 1
        },
        {
            "category": "Mythology",
            "type": "multiple",
            "difficulty": "easy",
            "question": "The ancient Roman god of war was commonly known as which of the following?",
            "correct_answer": "Mars",
            "incorrect_answers": [
                "Jupiter",
                "Mars",
                "Juno",
                "Ares"
            ],
            "id": 2
        },
        {
            "category": "Entertainment: Film",
            "type": "multiple",
            "difficulty": "easy",
            "question": "Who starred as Bruce Wayne and Batman in Tim Burton&#039;s 1989 movie &quot;Batman&quot;?",
            "correct_answer": "Michael Keaton",
            "incorrect_answers": [
                "George Clooney",
                "Val Kilmer",
                "Michael Keaton",
                "Adam West"
            ],
            "id": 3
        },
        {
            "category": "Entertainment: Video Games",
            "type": "multiple",
            "difficulty": "easy",
            "question": "In the Nintendo DS game &#039;Ghost Trick: Phantom Detective&#039;, what is the name of the hitman seen at the start of the game?",
            "correct_answer": "Nearsighted Jeego",
            "incorrect_answers": [
                "One Step Ahead Tengo",
                "Nearsighted Jeego",
                "Missile",
                "Cabanela"
            ],
            "id": 4
        },
        {
            "category": "Science & Nature",
            "type": "multiple",
            "difficulty": "easy",
            "question": "What does the letter &#039;S&#039; stand for in &#039;NASA&#039;?",
            "correct_answer": "Space",
            "incorrect_answers": [
                "Space",
                "Science",
                "Society",
                "Star"
            ],
            "id": 5
        },
        {
            "category": "Entertainment: Video Games",
            "type": "multiple",
            "difficulty": "easy",
            "question": "Who&#039;s the voice actor for Thrall in the Warcraft game series?",
            "correct_answer": "Chris Metzen",
            "incorrect_answers": [
                "Ben Affleck",
                "Jason Derulo",
                "Chris Metzen",
                "Jim Carrey"
            ],
            "id": 6
        },
        {
            "category": "Entertainment: Cartoon & Animations",
            "type": "multiple",
            "difficulty": "easy",
            "question": "The song &#039;Little April Shower&#039; features in which Disney cartoon film?",
            "correct_answer": "Bambi",
            "incorrect_answers": [
                "Cinderella",
                "Pinocchio",
                "The Jungle Book",
                "Bambi"
            ],
            "id": 7
        },
        {
            "category": "Entertainment: Film",
            "type": "multiple",
            "difficulty": "easy",
            "question": "Which actress danced the twist with John Travolta in &#039;Pulp Fiction&#039;?",
            "correct_answer": "Uma Thurman",
            "incorrect_answers": [
                "Kathy Griffin",
                "Uma Thurman",
                "Pam Grier",
                "Bridget Fonda"
            ],
            "id": 8
        },
        {
            "category": "Entertainment: Television",
            "type": "multiple",
            "difficulty": "easy",
            "question": "Who played Agent Fox Mulder in the TV sci-fi drama &quot;The X-Files&quot;?",
            "correct_answer": "David Duchovny",
            "incorrect_answers": [
                "David Duchovny",
                "Gillian Anderson",
                "Robert Patrick",
                "Mitch Pileggi"
            ],
            "id": 9
        },
        {
            "category": "Entertainment: Board Games",
            "type": "multiple",
            "difficulty": "easy",
            "question": "How many spaces are there on a standard Monopoly board?",
            "correct_answer": "40",
            "incorrect_answers": [
                "28",
                "55",
                "40",
                "36"
            ],
            "id": 10
        },
        {
            "category": "Science & Nature",
            "type": "multiple",
            "difficulty": "easy",
            "question": "Which is the longest bone in the human body? ",
            "correct_answer": "Femur",
            "incorrect_answers": [
                "Femur",
                "Scapula",
                "Fibula",
                "Ulna"
            ],
            "id": 11
        },
        {
            "category": "History",
            "type": "multiple",
            "difficulty": "easy",
            "question": "How many manned moon landings have there been?",
            "correct_answer": "6",
            "incorrect_answers": [
                "1",
                "3",
                "7",
                "6"
            ],
            "id": 12
        },
        {
            "category": "Science & Nature",
            "type": "multiple",
            "difficulty": "easy",
            "question": "How many planets are in our Solar System?",
            "correct_answer": "Eight",
            "incorrect_answers": [
                "Nine",
                "Eight",
                "Seven",
                "Ten"
            ],
            "id": 13
        },
        {
            "category": "Entertainment: Video Games",
            "type": "multiple",
            "difficulty": "easy",
            "question": "In Kingdom Hearts, how many members does Organization XIII have in total?",
            "correct_answer": "14",
            "incorrect_answers": [
                "10",
                "14",
                "13",
                "12"
            ],
            "id": 14
        },
        {
            "category": "Entertainment: Film",
            "type": "multiple",
            "difficulty": "easy",
            "question": "The Queen song `A Kind Of Magic` is featured in which 1986 film?",
            "correct_answer": "Highlander",
            "incorrect_answers": [
                "Highlander",
                "Flash Gordon",
                "Labyrinth",
                "Howard the Duck"
            ],
            "id": 15
        },
        {
            "category": "Entertainment: Film",
            "type": "multiple",
            "difficulty": "easy",
            "question": "What was Dorothy&#039;s surname in &#039;The Wizard Of Oz&#039;?",
            "correct_answer": "Gale",
            "incorrect_answers": [
                "Perkins",
                "Gale",
                "Day",
                "Parker"
            ],
            "id": 16
        },
        {
            "category": "Entertainment: Film",
            "type": "multiple",
            "difficulty": "easy",
            "question": "What was the title of the first Bond movie, released in 1962?",
            "correct_answer": "Dr. No",
            "incorrect_answers": [
                "From Russia with Love",
                "Goldfinger",
                "Dr. No",
                "Thunderball"
            ],
            "id": 17
        },
        {
            "category": "Science & Nature",
            "type": "multiple",
            "difficulty": "easy",
            "question": "Which of the following blood vessels carries deoxygenated blood?",
            "correct_answer": "Pulmonary Artery",
            "incorrect_answers": [
                "Pulmonary Vein",
                "Aorta",
                "Coronary Artery",
                "Pulmonary Artery"
            ],
            "id": 18
        },
        {
            "category": "Vehicles",
            "type": "multiple",
            "difficulty": "easy",
            "question": "Which of the following car manufacturers had a war named after it?",
            "correct_answer": "Toyota",
            "incorrect_answers": [
                "Honda",
                "Toyota",
                "Ford",
                "Volkswagen"
            ],
            "id": 19
        },
        {
            "category": "Entertainment: Video Games",
            "type": "multiple",
            "difficulty": "easy",
            "question": "In the 2002 video game &quot;Kingdom Hearts&quot;, how many Keyblades are usable?",
            "correct_answer": "18",
            "incorrect_answers": [
                "13",
                "16",
                "18",
                "15"
            ],
            "id": 20
        },
        {
            "category": "Entertainment: Japanese Anime & Manga",
            "type": "multiple",
            "difficulty": "easy",
            "question": "In &quot;Fairy Tail&quot;, what is the nickname of Natsu Dragneel?",
            "correct_answer": "The Salamander",
            "incorrect_answers": [
                "The Dragon Slayer",
                "The Salamander",
                "The Dragon",
                "The Demon"
            ],
            "id": 21
        },
        {
            "category": "Entertainment: Japanese Anime & Manga",
            "type": "multiple",
            "difficulty": "easy",
            "question": "Who is the true moon princess in Sailor Moon?",
            "correct_answer": "Sailor Moon",
            "incorrect_answers": [
                "Sailor Venus",
                "Sailor Mars",
                "Sailor Jupiter",
                "Sailor Moon"
            ],
            "id": 22
        },
        {
            "category": "General Knowledge",
            "type": "multiple",
            "difficulty": "easy",
            "question": "What is the nickname of the US state of California?",
            "correct_answer": "Golden State",
            "incorrect_answers": [
                "Sunshine State",
                "Golden State",
                "Bay State",
                "Treasure State"
            ],
            "id": 23
        },
        {
            "category": "Entertainment: Video Games",
            "type": "multiple",
            "difficulty": "easy",
            "question": "What is Gabe Newell&#039;s favorite class in Team Fortress 2?",
            "correct_answer": "Spy",
            "incorrect_answers": [
                "Heavy",
                "Medic",
                "Spy",
                "Pyro"
            ],
            "id": 24
        },
        {
            "category": "Entertainment: Video Games",
            "type": "multiple",
            "difficulty": "easy",
            "question": "What&#039;s the Team Fortress 2 Scout&#039;s city of origin?",
            "correct_answer": "Boston",
            "incorrect_answers": [
                "Sydney",
                "Boston",
                "Detroit",
                "New York"
            ],
            "id": 25
        },
        {
            "category": "Mythology",
            "type": "multiple",
            "difficulty": "easy",
            "question": "Which Greek &amp; Roman god was known as the god of music, truth and prophecy, healing, the sun and light, plague, poetry, and more?",
            "correct_answer": "Apollo",
            "incorrect_answers": [
                "Apollo",
                "Aphrodite",
                "Artemis",
                "Athena"
            ],
            "id": 26
        },
        {
            "category": "General Knowledge",
            "type": "multiple",
            "difficulty": "easy",
            "question": "Who is depicted on the US hundred dollar bill?",
            "correct_answer": "Benjamin Franklin",
            "incorrect_answers": [
                "Benjamin Franklin",
                "George Washington",
                "Abraham Lincoln",
                "Thomas Jefferson"
            ],
            "id": 27
        },
        {
            "category": "Entertainment: Video Games",
            "type": "multiple",
            "difficulty": "easy",
            "question": "In the videogame Bully, what is the protagonist&#039;s last name?",
            "correct_answer": "Hopkins",
            "incorrect_answers": [
                "Smith",
                "Hopkins",
                "Kowalski",
                "Crabblesnitch"
            ],
            "id": 28
        },
        {
            "category": "Entertainment: Video Games",
            "type": "multiple",
            "difficulty": "easy",
            "question": "How many differently shaped Tetris pieces are there?",
            "correct_answer": "7",
            "incorrect_answers": [
                "7",
                "5",
                "6",
                "8"
            ],
            "id": 29
        },
        {
            "category": "Animals",
            "type": "multiple",
            "difficulty": "easy",
            "question": "What is Grumpy Cat&#039;s real name?",
            "correct_answer": "Tardar Sauce",
            "incorrect_answers": [
                "Sauce",
                "Minnie",
                "Broccoli",
                "Tardar Sauce"
            ],
            "id": 30
        },
        {
            "category": "Entertainment: Video Games",
            "type": "multiple",
            "difficulty": "easy",
            "question": "In what year was Hearthstone released?",
            "correct_answer": "2014",
            "incorrect_answers": [
                "2011",
                "2014",
                "2013",
                "2012"
            ],
            "id": 31
        },
        {
            "category": "Entertainment: Video Games",
            "type": "multiple",
            "difficulty": "easy",
            "question": "In Super Mario Bros., who informs Mario that the princess is in another castle?",
            "correct_answer": "Toad",
            "incorrect_answers": [
                "Luigi",
                "Toad",
                "Yoshi",
                "Bowser"
            ],
            "id": 32
        },
        {
            "category": "Entertainment: Video Games",
            "type": "multiple",
            "difficulty": "easy",
            "question": "What is the maximum HP in Terraria?",
            "correct_answer": "500",
            "incorrect_answers": [
                "400",
                "1000",
                "100",
                "500"
            ],
            "id": 33
        },
        {
            "category": "Entertainment: Music",
            "type": "multiple",
            "difficulty": "easy",
            "question": "Who performed &quot;I Took A Pill In Ibiza&quot;?",
            "correct_answer": "Mike Posner",
            "incorrect_answers": [
                "Mike Posner",
                "Avicii",
                "Robbie Williams",
                "Harry Styles"
            ],
            "id": 34
        },
        {
            "category": "Entertainment: Cartoon & Animations",
            "type": "multiple",
            "difficulty": "easy",
            "question": "In the show &quot;Steven Universe&quot;, who are the main two employees of The Big Donut?",
            "correct_answer": "Sadie and Lars",
            "incorrect_answers": [
                "Steven and James",
                "Erik and Julie",
                "Bob and May",
                "Sadie and Lars"
            ],
            "id": 35
        },
        {
            "category": "Entertainment: Television",
            "type": "multiple",
            "difficulty": "easy",
            "question": "In &quot;Breaking Bad&quot;, Walter White is a high school teacher diagnosed with which form of cancer?",
            "correct_answer": "Lung Cancer",
            "incorrect_answers": [
                "Lung Cancer",
                "Prostate Cancer",
                "Brain Cancer",
                "Testicular Cancer"
            ],
            "id": 36
        },
        {
            "category": "Entertainment: Television",
            "type": "multiple",
            "difficulty": "easy",
            "question": "How many seasons did the Sci-Fi television show &quot;Stargate Universe&quot; have?",
            "correct_answer": "2",
            "incorrect_answers": [
                "10",
                "5",
                "2",
                "3"
            ],
            "id": 37
        },
        {
            "category": "Entertainment: Video Games",
            "type": "multiple",
            "difficulty": "easy",
            "question": "In &quot;Call Of Duty: Zombies&quot;, what is the name of the Pack-A-Punched Crossbow?",
            "correct_answer": "Awful Lawton",
            "incorrect_answers": [
                "Longinus",
                "Awful Lawton",
                "V-R11",
                "Predator"
            ],
            "id": 38
        },
        {
            "category": "Entertainment: Books",
            "type": "multiple",
            "difficulty": "easy",
            "question": "Which is NOT a book in the Harry Potter Series?",
            "correct_answer": "The House Elf",
            "incorrect_answers": [
                "The Chamber of Secrets",
                "The Prisoner of Azkaban",
                "The House Elf",
                "The Deathly Hallows"
            ],
            "id": 39
        },
        {
            "category": "Entertainment: Video Games",
            "type": "multiple",
            "difficulty": "easy",
            "question": "What is the hardest possible difficulty in &quot;Deus Ex: Mankind Divided&quot;?",
            "correct_answer": "I Never Asked For This",
            "incorrect_answers": [
                "Nightmare",
                "Extreme",
                "I Never Asked For This",
                "Guru"
            ],
            "id": 40
        },
        {
            "category": "Entertainment: Film",
            "type": "multiple",
            "difficulty": "easy",
            "question": "Which of these Movies was NOT released in 1996?",
            "correct_answer": "Gladiator",
            "incorrect_answers": [
                "Independence Day",
                "Gladiator",
                "The Rock",
                "Mission: Impossible"
            ],
            "id": 41
        },
        {
            "category": "Science: Mathematics",
            "type": "multiple",
            "difficulty": "easy",
            "question": "How many sides does a trapezium have?",
            "correct_answer": "4",
            "incorrect_answers": [
                "3",
                "4",
                "5",
                "6"
            ],
            "id": 42
        },
        {
            "category": "Entertainment: Video Games",
            "type": "multiple",
            "difficulty": "easy",
            "question": "Which member of the Velvet Room is not a playable character in Persona 4 Arena Ultimax?",
            "correct_answer": "Theodore",
            "incorrect_answers": [
                "Theodore",
                "Elizabeth",
                "Margaret",
                "Marie"
            ],
            "id": 43
        },
        {
            "category": "History",
            "type": "multiple",
            "difficulty": "easy",
            "question": "In what year was the M1911 pistol designed?",
            "correct_answer": "1911",
            "incorrect_answers": [
                "1911",
                "1907",
                "1899",
                "1917"
            ],
            "id": 44
        },
        {
            "category": "Entertainment: Comics",
            "type": "multiple",
            "difficulty": "easy",
            "question": "In &quot;Homestuck&quot; what is Dave Strider&#039;s guardian?",
            "correct_answer": "Bro",
            "incorrect_answers": [
                "Becquerel",
                "Bro",
                "Doc Scratch",
                "Halley"
            ],
            "id": 45
        },
        {
            "category": "General Knowledge",
            "type": "multiple",
            "difficulty": "easy",
            "question": "Which of the following blood component forms a plug at the site of injuries?",
            "correct_answer": "Platelets",
            "incorrect_answers": [
                "Red blood cells",
                "White blood cells",
                "Blood plasma",
                "Platelets"
            ],
            "id": 46
        },
        {
            "category": "Entertainment: Video Games",
            "type": "multiple",
            "difficulty": "easy",
            "question": "What Ultimate does Makoto Naegi, protagonist of Danganronpa: Trigger Happy Havoc, have? ",
            "correct_answer": "Ultimate Lucky Student",
            "incorrect_answers": [
                "Ultimate Lucky Student",
                "Ultimate Unlucky Student",
                "Ultimate Detective",
                "Ultimate Runner"
            ],
            "id": 47
        },
        {
            "category": "Entertainment: Film",
            "type": "multiple",
            "difficulty": "easy",
            "question": "In the movie &quot;V for Vendetta,&quot; what is the date that masked vigilante &quot;V&quot; urges people to remember?",
            "correct_answer": "November 5th",
            "incorrect_answers": [
                "November 6th",
                "November 4th",
                "November 5th",
                "September 5th"
            ],
            "id": 48
        },
        {
            "category": "Entertainment: Japanese Anime & Manga",
            "type": "multiple",
            "difficulty": "easy",
            "question": "In the Naruto manga, what is the last name of Tsunade?",
            "correct_answer": "Senju",
            "incorrect_answers": [
                "Senju",
                "Haruno",
                "Uzumaki",
                "Yamanaka"
            ],
            "id": 49
        },
        {
            "category": "Entertainment: Film",
            "type": "multiple",
            "difficulty": "easy",
            "question": "The 1996 film &#039;Fargo&#039; is primarily set in which US state?",
            "correct_answer": "Minnesota",
            "incorrect_answers": [
                "Minnesota",
                "North Dakota",
                "South Dakota",
                "Wisconsin"
            ],
            "id": 50
        }
    ]
};



let prevList = [];


function getRandomQuestion(){
    let quesNum = Math.random();
    quesNum*=100;
    quesNum=Math.floor(quesNum);
    quesNum%=50;
    // console.log(quesNum)
    return quesNum;
}

function checkAlreadyAndReturnFinal(){
    let found = undefined;
    let num = 0;
    do{
        num = getRandomQuestion();
        found = prevList.find(e => {
        // console.log("NUm" , e, num);
            return e === num;
        });
    }while((found!==undefined));
    return bank.results[num];
}

// Calling that async function
// changeDB();
let ques = {};

function changeQues(ques){
  document.getElementById("ques-id").innerHTML = "#" + prevList.length;
  document.getElementById("ques-question").innerHTML = " " + ques.question;
  document.getElementById("a").innerHTML = ques.incorrect_answers[0];
  document.getElementById("b").innerHTML = ques.incorrect_answers[1];
  document.getElementById("c").innerHTML = ques.incorrect_answers[2];
  document.getElementById("d").innerHTML = ques.incorrect_answers[3];
}


function showAnswers(){
  document.getElementById('ques-card-header').hidden = true;
  document.getElementById('ques-card-body').hidden = true;
  document.getElementById('ques-id').hidden = true;
  let restart = document.createElement('a');
  restart.setAttribute('href', 'index.html');
  restart.classList.add('btn');
  restart.classList.add('btn-primary');
  restart.innerText = "Retake Quiz";
  document.getElementById('ques-card').appendChild(restart);
  let count = 0;
  for(let i=0;i<prevList.length;i++){
    console.log(bank.results[prevList[i]]);

    let card = document.createElement('div');
    card.classList.add('card');

    let cardBody = document.createElement('div');
    cardBody.classList.add('card-body');
    
    let cardTitle = document.createElement('div');
    cardTitle.classList.add('card-title');
    cardTitle.classList.add('h4');
    
    let cardText = document.createElement('div');
    cardText.classList.add('card-text');
    cardText.classList.add('fw-bold');

    let span1 = document.createElement('span');
    let span2 = document.createElement('span');
    span1.classList.add('text-success');
    if(bank.results[prevList[i]].success){
      span2.classList.add('text-success');
      count++;
    } else span2.classList.add('text-danger');
    // span1.innerText = "Correct Answer : " + bank.results[prevList[i]].correct_answer;
    // span2.innerText = "Your Answer : " + bank.results[prevList[i]].urAnswer;
    let spanText1 = document.createTextNode('Correct Answer : ' + bank.results[prevList[i]].correct_answer + " |");
    let spanText2 = document.createTextNode(' Your Answer : ' + bank.results[prevList[i]].urAnswer);
    span1.appendChild(spanText1);
    span2.appendChild(spanText2);


    cardTitle.innerHTML = bank.results[prevList[i]].question;
    // let cardTitleText = document.createTextNode(bank.results[prevList[i]].question);
    // cardTitle.appendChild(cardTitleText);
    // cardTitle.innerText = bank.results[prevList[i]].question;

    cardText.appendChild(span1);
    cardText.appendChild(span2);
    cardBody.appendChild(cardTitle);
    cardBody.appendChild(cardText);
    card.appendChild(cardBody);
    document.getElementById('ques-card').appendChild(card);
  }
  document.getElementById('score-div').hidden = false;
  document.getElementById('score').innerText = count;

  
}



function getQuestion(){
  console.log("Length  : ", prevList.length);
  if(prevList.length<10){
    try {
    ques = checkAlreadyAndReturnFinal();
    prevList.push(ques.id-1);
    changeQues(ques);
    } catch(e) {
      console.log(e, ques);
    }
  } else {
    console.log("Quiz ends");
    showAnswers();
  }
}

function startQuiz(){
  getQuestion();
  document.getElementById('welcome').hidden = true;
  document.getElementById('line').hidden = true;
  document.getElementById('start-quiz').hidden = true;
  document.getElementById('ques-id').hidden = false;
  document.getElementById('ques-card-header').hidden = false;
  document.getElementById('ques-card-body').hidden = false;
}

function checkAnswer(ans) {
  bank.results[ques.id-1].urAnswer = ques.incorrect_answers[ans];
  if(ques.correct_answer===ques.incorrect_answers[ans]){
    bank.results[ques.id-1].success = true;
  } else bank.results[ques.id-1].success = false;
  console.log(bank.results[ques.id-1].success, bank.results[ques.id-1].urAnswer);
  getQuestion();
}